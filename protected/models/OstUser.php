...
class OstUser extends CActiveRecord {
    ...
    public function getroles() {
        ...
        $model = OstUserRole::model()->findAll(array("condition" => "user_id=$this->id AND role_code NOT LIKE 'dvsn%'"));

        if (sizeof($model) > 0) {

            foreach ($model as $row) {

                $role1[] = OstRefList::model()->getRefLabel(2, $row['role_code']);
            }
        ...
        $model2 = OstUserRole::model()->findAll(array("condition" => "user_id=$this->id AND role_code LIKE 'dvsn_%'"));

        if (sizeof($model2) > 0) {

            foreach ($model2 as $row2) {

                $role2[] = OstRefList::model()->getRefLabel(3, $row2['role_code']);
            }

            if ($output != '')
                $output.= '<br>';

            $output.= 'Division Administrator : ';
    ...
    public function getroles2($cat_id, $user_id) {
        ...
        if($cat_id == 2)
            $model = OstUserRole::model()->findAll(array("condition" => "user_id=$this->id AND role_code NOT LIKE 'dvsn_%'"));
        if($cat_id == 3)
            $model = OstUserRole::model()->findAll(array("condition" => "user_id=$this->id AND role_code LIKE 'dvsn_%'"));
