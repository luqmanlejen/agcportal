...
//                    $url = 'index.php?r=' . $row->url . '&menu_id=' . PortalElement::encrypt_decrypt('encrypt', $row->id);
                ...
                foreach ($model2 as $row2) {
                    ...
                    $output.= ' <ul><li><label for="'.$row2->id.'" style="display:none">id="'.$row2->id.'"</label><input id="'.$row2->id.'" type="radio">' . PortalTranslation::TranslateMenu($row2->id, $row2->answer) . '</li><br>';
    ...
    public static function GetMediaList($media_type) {
        ...
        if (strlen($media_type) <= 10) {
            ...
            $model = OstMedia::model()->findAll(array("condition" => "media_type='$media_type' AND parent_id=0 AND lang='en' AND status='psh' ORDER BY sort ASC"));
    ...
    public static function GetSitemapMenu() {
        ...
        if (isset($_GET['menu_id'])) {
            ...
            $output = PortalElement::LoopingToGetSitemapMenu($parent_menu, 0);
        }

        return $output;
    }

    public static function LoopingToGetSitemapMenu($parent_menu, $loopno) {

        $output = '';

        $model = OstMenuPortal::model()->findAll(array("condition" => "parent_menu=$parent_menu AND parent_lang=0 AND menu_type='header' AND hide_ind=0 ORDER BY sort ASC"));
        ...
        if (sizeof($model) > 0) {
            ...
            foreach ($model as $row) {
                ...
                $submenu = PortalElement::LoopingToGetSitemapMenu($row->id, $loopno);
