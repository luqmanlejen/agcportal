...
//                    $output = $model->remarks;
     ...
     public static function TranslateArchiveTitle($cat, $id, $title_en) {        
         //echo $cat; exit;
        $output = $title_en;

        if (Yii::app()->session['lang'] == 'my') {

            if($cat == 'Online Services' || $cat == 'Video Gallery' || $cat == 'Audio Gallery' || $cat == 'Public Slider' || $cat == 'Business Slider' || $cat == 'Background'){
                $model = OstMedia::model()->findByAttributes(array('parent_id' => $id, 'lang' => 'my'));
            } else if($cat == 'Manage Articles'){
                $model = OstArticles::model()->findByAttributes(array('parent_id' => $id, 'lang' => 'my'));
            } else if($cat == 'Speeches' || $cat == 'Annual Report' || $cat == 'Activities Reports' || $cat == 'Press Release'){
                $model = OstPublication::model()->findByAttributes(array('parent_id' => $id, 'lang' => 'my'));
            } else if($menu->title == 'Photo Gallery'){
                $model = OstPhotoAlbum::model()->findByAttributes(array('parent_id' => $id, 'lang' => 'my'));
            }
