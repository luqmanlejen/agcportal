    ...
    if ($search != 0) {
        if (sizeof($model) > 0) {
            ...
            foreach ($model as $row) {
                ...
                if(Yii::app()->session['lang'] === 'en'){
                    $unit_title = $row->unit_rel->UNIT;
                    if($unit_title == '-- Please Choose --'){
                        $unit_title = '';
                    }
                    //table header BI
                    //condition untuk hide table header apabila tiada data untuk dipaparkan di portal
                    if($row->PEN_UNIT_ID != 0){
                        $output .= '<table>
                            '. $unit_title .'
                            <thead>
							<tr>
                ...
                if(Yii::app()->session['lang'] === 'my'){
                    $unit_title = $row->unit_rel->UNIT_MY;
                    if($unit_title == '-- Sila Pilih --'){
                        $unit_title = '';
                    }
                    //table header BM
                    //condition untuk hide table header apabila tiada data untuk dipaparkan di portal
      ...
      if (sizeof($model4) > 0) {
                    foreach ($model4 as $row4) {
                        $gelaran = '';
                        $check_gelaran = HrStaffPeribadi::model()->findByPk($row4->PEN_STF_ID);
                        $nama_gelaran = LkpHrGelaranDk::model()->findByPk($check_gelaran->STF_TITLE);
                        if(sizeof($nama_gelaran) > 0){
                            $gelaran = $nama_gelaran->GDK_GELARAN;
                        }
                        ...
                        //condition untuk hide pengguna yang tiada unit dari dipaparkan di Portal
                        if (($row4->PEN_UNIT_ID == $row->PEN_UNIT_ID) &&  $row4->PEN_UNIT_ID!= 0){
                            $output .= '    <tr class="' . $class . '">
                
                                <td>' . $gelaran . ' ' . $row4->peribadi_rel->STF_NAMA . '</td>
