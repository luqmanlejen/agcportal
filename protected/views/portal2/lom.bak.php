...
$term = '
            ...
            <p align="center">Both printed and online versions reprint of the Laws of Malaysia Series were updated up to the date mentioned on the front page of each Law.</p>
        ';
...
//        $query1 = "SELECT * FROM ost_lom WHERE lom_lang='en' " . $sqlext . "  GROUP BY lom_no ORDER BY LENGTH(lom_no),lom_no ASC, lom_title DESC";
        ...
        //$query1 = "SELECT * FROM ost_lom WHERE lom_lang='en' GROUP BY lom_no ORDER BY lom_no + 0 $sortA";
//        $query1 = "SELECT * FROM ost_lom WHERE lom_lang='en' GROUP BY lom_no ORDER BY lom_no + 0 ASC";
//        if (isset(Yii::app()->session['lang']) && Yii::app()->session['lang'] === 'my') { //040815 luqman
//            $query1 = "SELECT * FROM ost_lom WHERE lom_lang='my' GROUP BY lom_no ORDER BY LENGTH(lom_no),lom_no ASC";
//        }
    }
...
//                                <td>' . $row->lom_no . '</td>
//                                <td>' . PortalTranslation::TranslateLomTitle($row->id, $row->lom_title) . '</td>
//                            </tr>';
//                    
//                }
            }
...
//                            else if($row->reprint == 0){
//                                $output .= '<a href="index.php?r=portal2/lom2&id='. $row3->id .'" class="linklom" target="_blank" class="linklom">' . PortalTranslation::TranslateLomTitle($row->id, $row->lom_title) . '</a>';
//                            }
                        ...
                        if ($row3->lom_no == $row->lom_no && $row3->reprint == 1) {
                                ...
                                if (Yii::app()->session['lang'] == 'my') {
                                    if($row3->lom_doc != ''){
                                        $output .= '<tr class="' . $class . '">
                                                <td>' . '' . '</td>
                                                <td><a href="index.php?r=portal2/lom2&id='. PortalTranslation::TranslateLomPdf($row3->id) .'" target="_blank" class="linklom">' . PortalTranslation::TranslateLomTitle($row3->id, $row3->lom_title) . '</a></td>                                    
                                            </tr>';
                                    } else {
                                        $output .= '<tr class="' . $class . '">
                                                <td>' . '' . '</td>
                                                <td>' . PortalTranslation::TranslateLomTitle($row3->id, $row3->lom_title) . '</td>                                    
                                            </tr>';
                                    }
                                }
                    ...
                    foreach ($model2 as $row2) {
                        ...
                        if ($row2->lom_no == $row->lom_no && $row2->online == 1) {
                                ...
                                if (Yii::app()->session['lang'] == 'en') {
                                    $output .= '<tr class="' . $class . '">
                                            <td>' . '' . '</td>
                                            <td><a href="index.php?r=portal2/lom2&id='. $row2->id.'" target="_blank" class="linklom">' . PortalTranslation::TranslateLomTitle($row2->id, $row2->lom_title) . '</a></td>                                    
                                        </tr>';
                                }
                                if (Yii::app()->session['lang'] == 'my') {
                                    if($row2->lom_doc != ''){
                                        $output .= '<tr class="' . $class . '">
                                                <td>' . '' . '</td>
                                                <td><a href="index.php?r=portal2/lom2&id='. PortalTranslation::TranslateLomPdf($row2->id) .'" target="_blank" class="linklom">' . PortalTranslation::TranslateLomTitle($row2->id, $row2->lom_title) . '</a></td>                                    
                                            </tr>';
                                    } else {
                                        $output .= '<tr class="' . $class . '">
                                                <td>' . '' . '</td>
                                                <td>' . PortalTranslation::TranslateLomTitle($row2->id, $row2->lom_title) . '</td>                                    
                                            </tr>';
                                    }
                                }
...
<?php $url = 'index.php?r=portal2/lom&menu_id=' . $_GET['menu_id']; ?>
            ...
            <tr>
                ...
                <th width='130'>
                    <?php 
                        if(isset($_GET['sortA'])) {
                            if($_GET['sortA'] == 'ASC'){
                                echo '<a href="index.php?r=portal2/lom&menu_id=VXlsMDlEclhJVXlGcUd6c0JreVhEUT09&sortA=DESC" style="color:white">'.$act.'</a>';
                            } else {
                                echo '<a href="index.php?r=portal2/lom&menu_id=VXlsMDlEclhJVXlGcUd6c0JreVhEUT09&sortA=ASC" style="color:white">'.$act.'</a>';
                            }
                        } else {
                            echo '<a href="index.php?r=portal2/lom&menu_id=VXlsMDlEclhJVXlGcUd6c0JreVhEUT09&sortA=DESC" style="color:white">'.$act.'</a>';
                        }
                        ...
                        if(isset($_GET['sort'])) {
                            if($_GET['sort'] == 'DESC'){
                                echo '<a href="index.php?r=portal2/lom&menu_id=VXlsMDlEclhJVXlGcUd6c0JreVhEUT09&sort=ASC" style="color:white">'.$title.'</a>';
                            } else {
                                echo '<a href="index.php?r=portal2/lom&menu_id=VXlsMDlEclhJVXlGcUd6c0JreVhEUT09&sort=DESC" style="color:white">'.$title.'</a>';
                            }
                        } else {
                            echo '<a href="index.php?r=portal2/lom&menu_id=VXlsMDlEclhJVXlGcUd6c0JreVhEUT09&sort=ASC" style="color:white">'.$title.'</a>';
                        }
