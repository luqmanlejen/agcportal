...
<script>
    ...
    function BrowseServer() {
        ...
        finder.selectActionFunction = function(fileUrl, data) {
            this.closePopup();
            var videolist = this.getSelectedFiles();
            var folder = this.getSelectedFolder();
            var folderurl = folder.getUrl();
            SetVideoList(folderurl, videolist);
        };
        finder.popup();
    }

    var global_no = 0;
    function SetVideoList(folderurl, videolist) {
        var videolist2 = String(videolist);
        var videolist_arr = videolist2.split(",");
        var output = '';
        for (var x = 0; x < videolist_arr.length; x++) {
            global_no++;
            var img = folderurl + videolist_arr[x];
            output += '<div class="photo photo_new photo_' + global_no + '">\n\
                            <img src="' + img + '">\n\
                            <input name="videolist[]" type="hidden" value="' + global_no + '"><br><br>\n\
                            <input name="videolist_sort_' + global_no + '" type="text" value="0" class="form-control" >\n\
                            <input name="videolist_imgurl_' + global_no + '" type="hidden" value="' + img + '" class="videolist">\n\
                            <span class="delete" onclick="DeleteVideo2(' + global_no + ')">X</span>\n\
                       </div>';
        }
        $('#videolist').prepend(output);
    }

    function DeleteVideo2(no) {
        $('.photo_' + no).remove();
...
<div class="form">
    ...
    <?php if (Yii::app()->controller->action->id == 'update') { ?>
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <tr>
                    <td colspan="2">
                        <input type="button" value="Upload Video" onclick="BrowseServer();" class="btn btn-primary" style="margin:0 !important;"/>                    
                        <div id="videolist" style="margin-top:10px;">
                            <?php
                            if (sizeof($model5) > 0) {
                                foreach ($model5 as $row5) {
                                    echo '<div class="photo">'
                                    . '<img src="' . $row5->url . '">'
                                    . '<span class="delete" onclick="DeleteVideo(' . $row5->id . ')">X</span>'
                                    . '<br><br>'
                                    . '<input name="videolist_old_sort_' . $row5->id . '" type="text" value="' . $row5->sort . '" class="form-control">'
                                    . '<input name="videolist_old[]" type="hidden" value="' . $row5->id . '">'
                                    . '</div>';
                                }
                            }
                            ?>
    ...
    <div class="clearfix form-actions no-margin">
        <a href="index.php?r=ostVideoAlbum/admin" class="btn btn-purple"><i class="ace-icon fa fa-arrow-left bigger-110s"></i>&nbsp;Back</a>&nbsp;&nbsp;
...
<script type="text/javascript">
    ...
    function DeleteVideo(id) {
        ...
        if (r == 1)
            window.location = 'index.php?r=OstVideoAlbum/deleteVideo&id=' + id;
...
<script>
    ...
    $(function() {
        ...
        CKEDITOR.replace('OstVideoAlbum_descr', {height: 300});
