    ...
    if(sizeof($model) > 0){
        ...
        foreach ($model as $row) {
            ...
            if ($row->lom_cat == 1) {
                ...
                $model7 = OstLom::model()->findAll(array('condition' => "lom_no=$row->lom_no"));
                
                //show first row of lom
                $lom_no_count = 0;
                if(Yii::app()->session['lang'] == 'en'){
                    foreach ($model7 as $row7){
                        
                        if($row7->lom_lang == 'en' && $row7->online != 1 && $row7->reprint != 1 && $row7->lom_rev != 1){
                            $output .= '<tr class="' . $class . '">
                                        <td>' . $row->lom_no . '</td>
                                        <td>';
                            if($row7->lom_doc != ''){
                                $output .= '<a href="index.php?r=portal2/lom2&id='. $row7->id .'" class="linklom" target="_blank" class="linklom">' . PortalTranslation::TranslateLomTitle($row7->id, $row7->lom_title) . '</a>';
                            } else {
                                $output .= '' . PortalTranslation::TranslateLomTitle($row7->id, $row7->lom_title) . '';
                            }
                            $output .= '</td></tr>';
                            $lom_no_count = 1;
                        }
